...
class Film implements iFilm_actor, iFilm_genre, iFilm_language, iFilm_comment
{
	...
	public function insert($db, $obj, $genres_ids, $languages_ids, $actors_ids)
	{
		try
		{
			...
			$stmt = $db->prepare("INSERT INTO films(title, release_year, plot, imdbID, runtime, poster, countries_id, directors_id) 
			...
			$this->setId($id); // nachdem ein neuer film eintrag erstellt worden ist, wird dessen id in der instanz gesetzt
				
			$this->insertFilm_genre($db, $genres_ids); // in der zwischentabelle films_genres wird ein eintrag erstellt
			$this->insertFilm_language($db, $languages_ids); // in der zwischentabelle films_languages wird ein eintrag erstellt
			$this->insertFilm_actor($db, $actors_ids); // in der zwischentabelle films_actors wird ein eintrag erstellt
			return true;
		}
	...
	public function deleteFilm($db, $comments_ids)
	{
		try
		{
			// Einträge in den Zwischentabellen werden gelöscht
			$this->deleteFilm_actor($db);	
			$this->deleteFilm_language($db);
			$this->deleteFilm_genre($db);
			$this->deleteFilm_comment($db);
			
			// eintrag aus der tabelle film wird gelöscht
	...
	public static function selectRand($db)
	{
		try
		{	
			$stmt = $db->query("SELECT films.id, title, poster, directors.director, GROUP_CONCAT(genre) AS genres FROM films
	...
	//////////////////////////////////////////
	/////// searchFilms
	// liefert als rückgabe allle filme aus der db wo der suchbegriff vorkommt
	// $q = suchbegriff
	public static function searchFilms($db, $q)
	{
		try
		{
			$stmt = $db->query("SELECT films.id, films.title AS Titel, directors.director AS Direktor, films.release_year AS Jahr, 
								GROUP_CONCAT(DISTINCT genre ORDER BY genre ASC) as Genre
								FROM films
								JOIN directors
	...
	// liefert als rückgabe allle filme aus der db wo der suchbegriff vorkommt
	// $q = suchbegriff
	public static function searchFilmsPagenation($db, $q, $page_first_result, $results_per_page)
	{
		try
		{
			$stmt = $db->query("SELECT films.id, films.title AS Titel, directors.director AS Direktor, films.release_year AS Jahr, 
	...
	public static function selectFilm($db, $film_id)
	{
		try
		{
			$sql = "SELECT  films.id, films.poster, title AS Titel, directors.director AS Direktor, 
					GROUP_CONCAT(DISTINCT genre ORDER BY title SEPARATOR ' / ') AS Genre, 
					GROUP_CONCAT(DISTINCT actor ORDER BY title SEPARATOR ' / ') AS Schauspieler, 
					release_year AS Jahr, country AS Land, 
					GROUP_CONCAT(DISTINCT language ORDER BY title SEPARATOR ' / ') AS Sprachen, 
					runtime AS Runtime, films.imdbID, plot
